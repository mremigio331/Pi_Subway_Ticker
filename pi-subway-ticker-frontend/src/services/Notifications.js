...
const notificationsReducer = (state, action) => {
  switch (action.type) {
    ...
    case NotificationConstants.CLEAR_PREVIOUS_NOTIFICATIONS:
      return state.filter(
        (m) => m.persistent || m.pathKey === action.payload.pathKey,
      );
...
export const AllNotificationsProvider = ({ children }) => {
  const [flashbarNotifications, dispatchFlashbarNotification] = useReducer(
  ...
  return (
    <AllNotificationsContext.Provider
      value={[flashbarNotifications, dispatchFlashbarNotification]}
    >
      {children}
    </AllNotificationsContext.Provider>
  );
